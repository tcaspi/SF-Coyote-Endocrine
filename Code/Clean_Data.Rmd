...
hormones_clean <- cleaned_data %>% 
  ...
  mutate(season = case_when(
    Month == 5 | Month == 6 | Month == 7 | Month == 8 | Month == 9 | Month == 10 ~ "Dry",
    Month == 11 | Month == 12 | Month == 1 | Month == 2 | Month == 3 | Month == 4  ~ "Wet"
  )) %>% 
  select(-c(Number, Date, Area)) %>%  
  relocate("SampleID", .after="Code") %>% relocate("FieldID", .after="SampleID") %>%
  relocate("Territory", .after="Site") %>% 
  relocate("season", .after="Day") %>% relocate("biol.season", .after="season") %>% 
