...
covariate_data <- glmm_df %>% select(all_of(covariate_columns)) %>% 
  rename(
    `Percent Impervious \n Surface Cover` = ISA,
    `Percent \n Urban Cover` = `Urban`,
   `Population \n Density` = Pop.Den.2020,
    `Housing \n Density` = Housing.Den.2020,
    `RRA Anthropogenic \n Food` = Anthropogenic)

head(covariate_data)

cor_matrix <- cor(covariate_data, use = "pairwise.complete.obs", method="kendall")
ggcorrplot(cor_matrix, 
           method = "square",
           type = "lower",    
           lab = TRUE,      
           lab_size = 3,      
           colors =c("#7570B3", "white", "#66A61E"))+
  theme_custom()+
  labs(x = NULL, y = NULL)

# ggsave("Figures/FigureS2.png", dpi=600, height=5, width=7)
```
...
dhglm_GC_percent_change <- map(comparisons_list, ~ do.call(avg_comparisons, c(list(dhglm_GC, comparison="lift", conf_level = 0.95), .x)))

glmm_GC_percent_change <- map(comparisons_list, ~ do.call(avg_comparisons, c(list(glmm_GC, comparison="lift",conf_level = 0.95), .x)))
...
dhglm_T3_percent_change <- map(comparisons_list, ~ do.call(avg_comparisons, c(list(dhglm_T3, comparison="lift", conf_level = 0.95), .x)))
glmm_T3_percent_change <- map(comparisons_list, ~ do.call(avg_comparisons, c(list(glmm_T3, comparison="lift", conf_level = 0.95), .x)))
...
```{r}
# Average marginal contrasts - change significance level as needed
dhglm_T4_percent_change <- map(comparisons_list, ~ do.call(avg_comparisons, c(list(dhglm_T4, comparison="lift", conf_level = 0.95), .x)))
glmm_T4_percent_change <- map(comparisons_list, ~ do.call(avg_comparisons, c(list(glmm_T4, comparison="lift", conf_level = 0.95), .x)))
...
## GC: Impervious surface cover
p.GC_DHGLM_ISA <- ggplot() +
  geom_ribbon(data = cond_GC_ISA_ID, aes(x = ISA, ymin = log(lower__), ymax = log(upper__)), alpha = 0.4, fill = "#CC7677") +
  geom_line(data = cond_GC_ISA_ID, aes(x = ISA, y = log(estimate__)), color = "#A4193D", size = 1) +
  geom_jitter(data = dhglm_df_GC, aes(x = ISA, y = log(GC)), color = "#A4193D", alpha = 0.5) +
  # Common graphics
  xlab(NULL) +
  ylab("Log Glucocorticoids (ng/g)") +
  theme_custom() +
  scale_x_continuous(expand = c(0.008, 0.05), limits = c(0, 100), breaks = c(0, 25, 50, 75, 100)) +
  scale_size_continuous(range = c(1, 3)) +
  scale_y_continuous(limits = c(1.5, 7.5), breaks = c(2, 3, 4, 5, 6, 7))
  

## T3: Impervious surface cover
p.T3_DHGLM_ISA <- ggplot() +
  geom_ribbon(data = cond_T3_ISA_ID, aes(x = ISA, ymin = log(lower__), ymax = log(upper__)), alpha = 0.4, fill = "#FFA15D") +
  geom_line(data = cond_T3_ISA_ID, aes(x = ISA, y = log(estimate__)), color = "#FF7522", size = 1, linetype = "dashed") +
  geom_jitter(data = dhglm_df, aes(x = ISA, y = log(T3)), color = "#FF7522", alpha = 0.5) +
  # Common graphics
  xlab(NULL) +
...
## T4: Impervious surface cover
p.T4_DHGLM_ISA <- ggplot() +
  geom_ribbon(data = cond_T4_ISA_ID, aes(x = ISA, ymin = log(lower__), ymax = log(upper__)), alpha = 0.4, fill = "#55C235") +
  geom_line(data = cond_T4_ISA_ID, aes(x = ISA, y = log(estimate__)), color = "#2D661C", size = 1) +
  geom_jitter(data = dhglm_df, aes(x = ISA, y = log(T4)), color = "#2D661C", alpha = 0.5) +
  
  # Common graphics
  xlab("Percent Impervious Surface Cover") +
...
## GC: Diet
p.GC_DHGLM_Diet <- ggplot() +
  geom_ribbon(data = cond_GC_ID, aes(x = Anthropogenic, ymin = log(lower__), ymax = log(upper__)), alpha = 0.4, fill = "#CC7677") +
  geom_line(data = cond_GC_ID, aes(x = Anthropogenic, y = log(estimate__)), color = "#A4193D", size = 1) +
  geom_jitter(data = dhglm_df_GC, aes(x = Anthropogenic, y = log(GC)), color = "#A4193D", alpha = 0.5) +
  # Common graphics
  xlab(NULL) +
  ylab("Log Glucocorticoids (ng/g)") +
  theme_custom() +
  scale_x_continuous(expand = c(0.001, 0.01), limits = c(-0.01, 1.01), breaks = c(0, .25, .50, .75, 1)) +
  scale_size_continuous(range = c(1, 3)) +
  scale_y_continuous(limits = c(1.5, 7.5), breaks = c(2, 3, 4, 5, 6, 7))

## T3: Diet
p.T3_DHGLM_Diet <- ggplot() +
  geom_ribbon(data = cond_T3_ID, aes(x = Anthropogenic, ymin = log(lower__), ymax = log(upper__)), alpha = 0.4, fill = "#FFA15D") +
  geom_line(data = cond_T3_ID, aes(x = Anthropogenic, y = log(estimate__)), color = "#FF7522", size = 1) +
  geom_jitter(data = dhglm_df, aes(x = Anthropogenic, y = log(T3)), color = "#FF7522", alpha = 0.5) +
  # Common graphics
  xlab(NULL) +
  ylab("Log Triiodothyronine (ng/g)") +
  theme_custom() +
...
## T4: Diet
p.T4_DHGLM_Diet <- ggplot() +
  geom_ribbon(data = cond_T4_ID, aes(x = Anthropogenic, ymin = log(lower__), ymax = log(upper__)), alpha = 0.4, fill = "#55C235") +
  geom_line(data = cond_T4_ID, aes(x = Anthropogenic, y = log(estimate__)), color = "#2D661C", size = 1) +
  geom_jitter(data = dhglm_df, aes(x = Anthropogenic, y = log(T4)), color = "#2D661C", alpha = 0.5) +
  # Common graphics
  xlab("Proportion Anthropogenic Food") +
  ylab("Log Thyroxine (ng/g)") +
  theme_custom() +
  scale_x_continuous(expand = c(0.001, 0.01), limits = c(-0.01, 1.01), breaks = c(0, .25, .50, .75, 1)) +
  scale_size_continuous(range = c(1, 3)) +
...
## Arrange plots
ggarrange(p.GC_DHGLM_Diet, p.GC_DHGLM_ISA,
          p.T3_DHGLM_Diet, p.T3_DHGLM_ISA,
          p.T4_DHGLM_Diet, p.T4_DHGLM_ISA,
          ncol=2, nrow=3,
          labels = c("a", "d", "b", "e", "c", "f"),
          label.x = 0.13)

# ggsave("Figures/Figure2.png", dpi=600, height=7, width=6)
```
...
p.type.GC <- ggplot()+
  ggridges::geom_density_ridges(data = mu_posteriors_GC, 
                                aes(x = value,
                                    ...
                                    height = ..density.., 
                                    scale = 1.5), alpha = 0.6, fill= "#A4193D",
                                stat = "density", trim=T)+
  geom_point(data = mu_posteriors_GC[!duplicated(mu_posteriors_GC$Individual),],
             aes(x = meanlogGC, y = as.factor(Individual), shape = Sex, fill=Breeder),
             size=2.5, color="black", show.legend = c(size=F))+
...
p.type.T3 <- ggplot()+
  ggridges::geom_density_ridges(data = mu_posteriors_T3, 
                                aes(x = (value),
                                    ...
                                    height = ..density.., 
                                    scale = 1.5), alpha = 0.8,
                                fill = "#ff6200", stat = "density", trim=T)+
  geom_point(data = mu_posteriors_T3[!duplicated(mu_posteriors_T3$Individual),],
             aes(x = (meanlogT3), y = as.factor(Individual), shape = Sex,
                 fill=Breeder), color = "black", size=2.5, show.legend = c(size=F))+
...
p.riiv.T3 <- ggplot()+
    ...
    geom_point(data = sd_posteriors_T3[!duplicated(sd_posteriors_T3$Individual),],
               aes(x = (meanrIIV), y = as.factor(Individual), shape = Sex,
                   fill = Breeder), color = "black", size = 2.5, 
               show.legend = c(size=F))+
...
p.type.T4 <- ggplot()+
  ggridges::geom_density_ridges(data = mu_posteriors_T4, 
                                aes(x = (value),
                                    ...
                                    height = ..density.., 
                                    scale = 1.5), alpha = 0.6,
                                 fill="#2D661C", stat = "density", trim=T)+
  geom_point(data = mu_posteriors_T4[!duplicated(mu_posteriors_T4$Individual),],
             aes(x = (meanlogT4), y = as.factor(Individual), shape = Sex,
                 fill=Breeder), color = "black", size=2.5, show.legend = c(size=F))+
...
ggarrange(p.type.GC, p.type.T3, p.type.T4,
          p.riiv.GC, p.riiv.T3, p.riiv.T4,
          ncol=3, nrow=2,
          labels = c("a", "b", "c", "d", "e", "f"),
          label.x = 0.28)

# ggsave("Figures/Figure4.png", width=10, height = 7, dpi=600)
```
...
value_cor.GC.riiv <- bquote(italic(r) == .(round(cor.GC.riiv$estimate, 2)))
value_p.GC.riiv   <- bquote(italic(p) == .(round(cor.GC.riiv$p.value, 2)))
value_cor.T3.riiv <- bquote(italic(r) == .(round(cor.T3.riiv$estimate, 2)))
value_p.T3.riiv   <- bquote(italic(p) == .(round(cor.T3.riiv$p.value, 2)))
value_cor.T4.riiv <- bquote(italic(r) == .(round(cor.T4.riiv$estimate, 2)))
value_p.T4.riiv   <- bquote(italic(p) == .(round(cor.T4.riiv$p.value, 2)))
...
# ggsave("Figures/FigureS6.png", dpi=600, height=2.5, width=8)
```
